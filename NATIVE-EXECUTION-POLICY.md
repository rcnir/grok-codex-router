...
**Not deployed.** Human Gate 1 created exactly one isolated BOX pilot; no Turn was
started. Production activation is now split into Gate 2A build/stage, Gate 2B
Runtime/Codex promotion and Gate 2C router/host activation. This document still
records only the Codex dynamic-only boundary; it does not authorize any of those
mutations or provider/model inference.
